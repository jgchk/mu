...
>
  {#if $tracksQuery.data}
    ...
    {#if previousTracks.length}
      <div>
        <h6 class="mb-1 ml-1 font-bold text-gray-400">Previous</h6>
        <TrackList
    ...
    <div use:autoscroll>
      <h6 class="text-primary-600 mb-1 ml-1 font-bold">Now playing</h6>
      {#if nowPlayingTrack}
        <TrackList
    ...
    {#if nextTracks.length}
      <div>
        <h6 class="mb-1 ml-1 font-bold text-gray-400">Next</h6>
        <TrackList
          tracks={nextTracks}
          on:favorite={(e) =>
            $favoriteMutation.mutate({ id: e.detail.track.id, favorite: e.detail.favorite })}
