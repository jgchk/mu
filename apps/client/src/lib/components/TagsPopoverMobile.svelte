...
<div class="flex items-center gap-1">
  <IconButton kind="text" tooltip="Back" on:click={() => dispatch('close')}>
    <ArrowLeftIcon />
  </IconButton>
  <div class="font-medium">Edit tags</div>
</div>
...
<div class="max-h-[calc(100vh/3)] overflow-auto" tabindex="-1">
  ...
  {:else if $tagsQuery.error}
    ...
    >
      <span class="text-error-500" use:tooltip={{ content: $tagsQuery.error.message }}>Error</span> loading
      tags. Retry?
    </Button>
  {:else}
