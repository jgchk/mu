...
const getVQD = async (query: string, ia = 'web') => {
  const response = await got('https://duckduckgo.com/', { searchParams: { q: query, ia } }).text()

  const vqd = VQD_REGEX.exec(response)?.[1]

  if (vqd === undefined) throw new Error(`Failed to get the VQD for query "${query}"`)
...
export const search = async (query: string): Promise<SearchResults> => {
  ...
  const res = await got('https://links.duckduckgo.com/d.js', { searchParams: queryObject }).text()
